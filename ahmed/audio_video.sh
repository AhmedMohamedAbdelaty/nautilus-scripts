...
for selected_file in "${selected_files[@]}"; do
    ...
    then
        ...
        for input_audio in $audio_files; do
            ...
            ffmpeg -loop 1 -i "$image_file" -i "$input_audio" -c:v libx264 -preset fast -tune stillimage -c:a copy -shortest "$output_video"
        ...
        if [[ "$selected_file" == *.mp3 || "$selected_file" == *.wav || "$selected_file" == *.ogg ]]; then
            ...
            # Convert audio file to video
            ffmpeg -loop 1 -i "$image_file" -i "$selected_file" -c:v libx264 -preset fast -tune stillimage -c:a copy -shortest "$output_video"
